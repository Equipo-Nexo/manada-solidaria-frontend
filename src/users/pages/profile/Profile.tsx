...
function Profile() {
  ...
  const [selectedRole, setSelectedRole] = useState<RoleInformation | null>(
    null,
  );
  ...
  const [updateUserProfile, { isLoading: isUpdatingProfile }] =
    useUpdateUserProfileMutation();
  const isSavingPhoto =
    isGettingPresignedUrl || isUploadingImage || isUpdatingProfile;

  const activeRoles = editableRoles.filter(
    (roleCode) =>
  ...
  const updateProfilePhoto = async (file: File) => {
    ...
    try {
      ...
      toaster.error(
        "No pudimos actualizar la foto de perfil",
      ...
      <S.SwitchGroup>
        <S.SwitchRow>
          <S.SwitchLabelContent>
            <S.RoleIcon $size="large">
              <Icon {...iconProps} aria-hidden="true" />
            </S.RoleIcon>
            <S.RoleName>{Rolename}</S.RoleName>
            <S.InfoIcon
              type="button"
              aria-label={`Información sobre el rol de ${Rolename}`}
              onClick={() => handleRoleInformation(Rolename)}
            >
  ...
  return (
    <S.MainContainer>
      ...
      <BottomSheet
        ...
        ariaLabel={
          selectedRole
            ? `Información sobre el rol ${selectedRole.name}`
            : "Información del rol"
        }
      >
      ...
      <S.OptionsContainer>
        <S.RolesContainer>
          ...
          <S.Description>
            Activá los roles que querés cumplir en la red. Tocá el ícono de
            información para saber en qué consiste cada uno.
          </S.Description>
          <S.RolesList>
            {SwitchRoleComponent("Rescatista", HandHeart, {
              variante: "outlined",
            })}
        ...
        <S.ItemsMainContainer>
          ...
          <S.Description>
            Accedé a tus publicaciones y actualizá tus datos personales cuando
            lo necesites.
          </S.Description>
          <S.ItemsList>
            {ItemComponent(
              UserIcon,
              "Datos personales",
              "Corroborá y editá tus datos personales",
              "/mi-perfil/datos-personales",
            )}
            {ItemComponent(
              HistoryIcon,
              "Mis publicaciones",
              "Editá y eliminá tus publicaciones",
              "/mis-publicaciones",
            )}
          </S.ItemsList>
        </S.ItemsMainContainer>
        <S.ItemsMainContainer>
          ...
          <S.ItemsList>
            {ItemComponent(
              ...
              "Configurá el acceso con passkey",
              "/mi-perfil/seguridad",
            )}
          </S.ItemsList>
        </S.ItemsMainContainer>
